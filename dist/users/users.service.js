...
let UsersService = class UsersService {
    ...
    async updateUserPreferences(id, input) {
        await this.findUserById(id);
        return this.prisma.user.update({
            where: { id },
            data: input,
    ...
    async updateUserPassword(id, input) {
        const user = await this.prisma.user.findUnique({
            where: { id },
        ...
        if (!user)
            throw new common_1.NotFoundException('User not found');
        if (!user.password)
            throw new common_1.BadRequestException('Esta conta não usa senha local');
    ...
    async updateUserAvatar(id, file) {
        ...
        await this.findUserById(id);
        const stored = await this.storage.saveFile(file.buffer, file.originalname, 'avatars');
        return this.prisma.user.update({
            where: { id },
